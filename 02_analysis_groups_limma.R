# Group-wise NPQ analysis: limma::lmFit + contrasts.fit + eBayes (see Reference_Code/premodiALS_NULISA/code/02_analysis_groups.R).
### Libraries
...
# Per-fluid stats via limma::lmFit (+ contrasts.fit / eBayes), aligned with premodiALS Reference_Code pattern.
run_stats_for_fluid <- function(df, fluid, value_col = "NPQ",
                                adjust = FALSE,
                                covariates = c("age", "sex")) {
  df_f <- df %>%
    dplyr::filter(SampleMatrixType == fluid) %>%
    dplyr::filter(!is.na(.data[[value_col]]))

  if (nrow(df_f) == 0L) {
    warning("No rows for fluid: ", fluid)
    return(list(anova = tibble::tibble(), pairwise = tibble::tibble(), data = df_f))
  }

  miss_cov <- setdiff(covariates, names(df_f))
  if (length(miss_cov) > 0L) {
    stop("Covariates not in data: ", paste(miss_cov, collapse = ", "))
  }

  data_wide <- df_f %>%
    dplyr::select(SampleName, Target, dplyr::all_of(value_col)) %>%
    tidyr::pivot_wider(names_from = SampleName, values_from = dplyr::all_of(value_col)) %>%
    as.data.frame()
  rownames(data_wide) <- data_wide$Target
  exp_matrix <- as.matrix(data_wide[, -1L, drop = FALSE])

  meta <- df_f %>%
    dplyr::select(SampleName, type, dplyr::all_of(covariates)) %>%
    dplyr::distinct(SampleName, .keep_all = TRUE) %>%
    dplyr::slice(match(colnames(exp_matrix), .data$SampleName))

  if (anyNA(meta$SampleName) || nrow(meta) != ncol(exp_matrix)) {
    warning("Sample alignment mismatch for fluid ", fluid)
    return(list(anova = tibble::tibble(), pairwise = tibble::tibble(), data = df_f))
  }

  formula_str <- if (adjust) {
    paste("~ 0 + type +", paste(covariates, collapse = " + "))
  } else {
    "~ 0 + type"
  }

  design <- stats::model.matrix(stats::as.formula(formula_str), data = meta)
  colnames(design) <- gsub("^type", "", colnames(design))

  lev <- levels(droplevels(factor(meta$type)))
  if (length(lev) < 2L) {
    warning("Need at least two type levels for fluid ", fluid)
    df_out <- df_f
    if (adjust) {
      df_out$NPQ_adj <- NA_real_
    }
    return(list(anova = tibble::tibble(), pairwise = tibble::tibble(), data = df_out))
  }

  contrast_pairs <- utils::combn(lev, 2L, simplify = FALSE)
  contrast_strings <- vapply(contrast_pairs, function(x) paste(x[1L], x[2L], sep = "-"), character(1L))

  fit <- limma::lmFit(exp_matrix, design)
  contrast_matrix <- limma::makeContrasts(contrasts = contrast_strings, levels = design)
  fit2 <- limma::contrasts.fit(fit, contrast_matrix)
  fit2 <- limma::eBayes(fit2)

  tt_anova <- limma::topTable(fit2, number = Inf, sort.by = "none") %>%
    tibble::as_tibble(rownames = "Target")

  anova_res <- if ("F" %in% names(tt_anova)) {
    tt_anova %>%
      dplyr::arrange(dplyr::desc(.data[["F"]])) %>%
      dplyr::mutate(
        p = .data[["P.Value"]],
        f_stat = .data[["F"]]
      )
  } else {
    # With one contrast (e.g., two groups), topTable does not return F.
    tt_anova %>%
  ...
  anova_res <- anova_res %>%
    dplyr::mutate(
      Fluid = fluid,
      Adjustment = ifelse(adjust, "ADJ", "UNADJ"),
      Effect = "type"
    )

  pairwise_res <- lapply(contrast_strings, function(cm) {
    limma::topTable(fit2, coef = cm, number = Inf, sort.by = "none") %>%
      tibble::as_tibble(rownames = "Target") %>%
      dplyr::mutate(contrast = cm)
  }) %>%
    dplyr::bind_rows() %>%
    tidyr::separate(contrast, into = c("group1", "group2"), sep = "-") %>%
    dplyr::rename(p = P.Value) %>%
    dplyr::group_by(.data$group1, .data$group2) %>%
    dplyr::mutate(p.adj = stats::p.adjust(.data$p, method = "BH")) %>%
    dplyr::ungroup() %>%
    rstatix::add_significance("p.adj") %>%
    dplyr::mutate(Fluid = fluid, Adjustment = ifelse(adjust, "ADJ", "UNADJ"))

  if (adjust) {
    num_ix <- vapply(meta, is.numeric, logical(1L))
    cov_mat <- as.matrix(meta[, num_ix, drop = FALSE])
    des_type <- stats::model.matrix(~ 0 + type, data = meta)
    colnames(des_type) <- gsub("^type", "", colnames(des_type))

    adjusted_exp <- if (ncol(cov_mat) > 0L) {
      limma::removeBatchEffect(exp_matrix, design = des_type, covariates = cov_mat)
    } else {
      limma::removeBatchEffect(exp_matrix, design = des_type)
    }

    adj_df <- as.data.frame(adjusted_exp)
    adj_df$Target <- rownames(adjusted_exp)
    adj_long <- tidyr::pivot_longer(
      adj_df,
      cols = -Target,
      names_to = "SampleName",
      values_to = "NPQ_adj"
    )
    df_f <- df_f %>% dplyr::left_join(adj_long, by = c("Target", "SampleName"))
  }

  pairwise_res <- pairwise_res %>%
    dplyr::mutate(p_label = format_p(.data$p, digits = 3L))

  list(anova = anova_res, pairwise = pairwise_res, data = df_f)
}
...
plot_top_proteins_violin <- function(df, stats_list, td, fluid, top_n = 15, subtype = FALSE, adjusted = FALSE) {
  ...
  pairwise_res <- get_pairwise_sig(
    stats_list, fluid, top_proteins,
    df_top = df_for_pairwise,
    adjusted = adjusted
  )
